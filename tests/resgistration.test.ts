...
var password = await browser.getText(registrationpage.password);
var email = await browser.getValue(registrationpage.email);
const records = [
    {id: 'email',  lang: email},
    {id: 'pass', lang: password}
    ];
...
await browser.waitUntilElementIsVisible(registrationpage.state,60000);
...
await browser.waitUntilElementIsVisible(registrationpage.country,60000);
